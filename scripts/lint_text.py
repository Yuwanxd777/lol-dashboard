...
歷史抓到過的真 bug（別讓它們回來）：
  ...
  「持續秒」「在秒內」（2026-08-15：CDragon 落後 DDragon 時 skills 快取把破句卡了整個版本週期）

★中文排版規則不可以套到英文行。wiki_patches.js 混著未翻完的英文原文（翻譯守門定案），
  英文的「40% (based on stacks)」百分號後接空格＋括號是正確寫法。判斷要看**內容**不是看檔名。
"""
...
RULES = [
    ...
    # 值被剝掉後留下沒有數字的句子：「持續秒」「在秒內」「最多名士兵」。
    # ⚠ 只認「期待數值的動詞／副詞＋單位」，別放寬成「單位字前面不是數字」——那樣會把
    #   「一層堆層」「三次」「地點」「數秒」全部誤判（2026-08-15 第一版規則 208 筆裡只有 4 筆是真的）。
    #   現行版本套過全庫 173 位英雄，命中 10 筆全是真破句、零誤判。與 fetch_skills.HOLE_RE 同一套。
    #   「發／顆／隻」試過並移除：「持續發射兩列飛彈」會被「持續」＋「發」打到。
    #   單位字後面也要排掉「其實是另一個詞的第一個字」：秒表(Stopwatch)、個人檔案、名字。
    ("數值被剝掉（單位前無數字）",
     re.compile(r"(?:持續|在|最多|最少|共|達|增加|獲得|回復|提升|縮短|延長|緩速|賦予)\s*"
                r"(?:秒(?![表錶])|層|次|碼|％|名(?![字稱單義])|個(?![人別體性月案]))(?![數量])"), ERR),
    ("數字與 % 之間有空格",        re.compile(r"\d\s+%"), ERR),   # 「25 % 跑速」→ 應為「25% 跑速」
    # 「30% （+3% AP）」→「30%（+3% AP）」是**中文排版**規則：英文的「50% (levels 1-18)」
    # 百分號後接空格＋括號本來就正確。不能只靠檔名判斷——wiki_patches.js 裡有大量未翻完的
    # 英文行（翻譯守門定案：混雜整段顯示英文原文），2026-08-15 體檢的 11 筆「錯誤」全是這種誤報。
    # 改成看括號本身：全形括號、或半形括號裡有中文，才算中文語境。
    ("% 與括號之間有空格",         re.compile(r"%\s+（|%\s+\([^)]*[一-鿿]"), ERR),
