...
PHRASE = [
    ...
    # ── 舊翻譯規則留下的壞句（2026-08-15 體檢「數值被剝掉」抓到，永久快取不會自己重生）──
    # 詞庫端已補 ("per hit","每次命中")，但既有資料要靠這裡補套。
    ("能量回復秒命中：", "每次命中回復能量："),        # Energy Restore Per Hit
    ("額外範圍現在層額外範圍從死神降臨。",             # now stacks additional range from …
     "額外範圍現在會與死神降臨提供的額外範圍疊加。"),
    ("被動獲得層每 40 秒從 60。",                     # passive gains stacks every 60s ⇒ 40s
     "被動的層數改為每 40 秒獲得一層（原為每 60 秒）。"),
]
...
def fix_html(s):
    ...
    s = re.sub(r"%i:\w+%", "", s)
    s = re.sub(r"%{2,}", "%", s)
    s = re.sub(r"[ \t]+(?=[。，、：；！？）])", "", s)     # 標點前空白（同下面純文字路徑；標籤不受影響）
    s = re.sub(r"[ \t]{2,}", " ", s)
    return s, s != o
...
def fix(s, en=False):
    ...
    # ⚠「孤零零的 %」那條規則已刪除（2026-08-15）：它原本要清圖示被剝掉後留下的 %，但實測套到
    #   現有資料上**每一次命中都是誤刪有意義的百分號**——「每 % 已損失生命值」→「每 已損失生命值」、
    #   「命中觸發 % 最大生命值傷害」→「命中觸發 最大生命值傷害」、「(+ 60 % AP)」→「(+ 60 AP)」。
    #   前後有空白的 % 在中英文裡都可能是正經寫法，光看空白分不出來，寧可不清。
    #   （下面那條 `(\d)\s+%` 會把「60 % AP」正規化成「60% AP」，那是純排版、不動語意。）
    s = re.sub(r"(AP|AD|物攻|魔攻|暴擊率|攻速)\s*%(?=[一-鿿])", r"\1", s)  # 「+10% AP% 最大魔力」尾巴那個 % 是模板的
    #   ⚠ 要求後面接中文才算：句尾的「115% + 暴擊率%」那個 % 是單位本身，刪掉會掉語意
    ...
    # 「30% （+3% AP）」→「30%（+3% AP）」是**中文排版**規則。英文的「50% (levels 1-18)」百分號後
    # 接空格＋括號本來就是正確寫法（2026-07-28 定案），但光看「是不是 patches_en.js」不夠——
    # wiki_patches.js 裡有大量**未翻完的英文行**（翻譯守門定案：混雜整段顯示英文原文），
    # 用檔名判斷會把它們一起擠掉（2026-08-15 預覽時抓到 11 筆）。改成看**括號本身**：
    # 全形括號、或半形括號裡有中文，才算中文語境。
    if not en:
        s = re.sub(r"%\s+(?=（)", "%", s)
        s = re.sub(r"%\s+(?=\([^)]*[一-鿿])", "%", s)
    s = re.sub(r"[ \t]+(?=[。，、：；！？）])", "", s)     # 標點前空白（標籤剝掉留下的；371 筆，2026-09-07）
    #   ⚠ 用 [ \t] 不用 \s：這些文本用 \n 代表 <br>，\s 會把兩行黏成一行
