...
安全網：執行前先把完整歷史打包成 bundle 放到倉庫上一層的 _備份 資料夾（Desktop\CLAUDE專案\_備份\LOL儀表板_git_backup_日期.bundle，可隨時還原）。
...
BK_DIR = os.path.join(os.path.dirname(ROOT), "_備份")   # 倉庫上一層（Desktop\CLAUDE專案）的 _備份，跟著專案走
os.makedirs(BK_DIR, exist_ok=True)
bundle = os.path.join(BK_DIR, f"LOL儀表板_git_backup_{day}.bundle")
...
run("add", "-A")
run("commit", "-m", f"squashed history {day} (content unchanged; full history in _備份 bundle)")
