...
# 這次局數有人讀 ⇒ 兩側的去重、±1 天合併、48h 寬限、我們側多收 slack（#171 起 5h）、留給逐聯賽落後叫的那幾天，每一條都配「拿掉就翻面」的對照。
...
def DC_SUITE(M, eq):
    ...
    try:
        ...
        # ⑧ 我們側多收 slack：同一局 wiki 07:58（剛好在那一刀前）、我們 08:03（刀後）⇒ 不報；多收改 0 ⇒ 誤報
        W8 = wk([(LCS, "2026-09-14 07:58:00", "A", "B", "1")])
        r8 = mk([("2026-09-14 08:03:00", "LCS", "A", "B", 1)])
        eq(DC(r8, W8), ("ok", []), "㉙⑧ 兩邊開賽時間跨在 48h 那一刀上 ⇒ 我們側多收 slack 吸掉、不誤報")
        ...
        # ⑳ ±1 天合併時相鄰天兩側同一刀（#170）。48h 那一刀 09-14 08:00、我們那一刀 09-14 13:00（slack 5h；#170 當時是 3h＝11:00）。
        #    重現 08-19 00:25 那班：當天真的少 2 局，隔天 3 局落在兩刀之間（我們收了、wiki 48h 還沒收）⇒ #168 寫法 5+1 ≤ 3+4 抵掉
        ...
        #    ⇒ 不報（09:00 那局在寬限內，不可以因為合併改用我們那一刀就被算成少局）
        ...
        # ㉑ 補檔局的假時鐘（#171）：fetch_fill 把 gol.gg 的局排成 min(9+3×系列序+局序, 23):局序×7:00，會比 wiki 晚好幾小時。
        #    實例 08-16 22:00 那班：LPL 08-14 JDG vs AL G3 我們 17:14:00、wiki 13:29（晚 3h45m）⇒ slack 3h 時誤報 45 分鐘。
        #    這裡用理論最大值那一級（晚 4h03m，LCK 09-13 第 1 系列 G4 的假時鐘差）：wiki 07:58（剛好在 48h 那一刀前）、我們 12:01:00。
        eq(M.DAYCOUNT_SLACK_H, 5, "㉙㉑ slack 5 小時（補檔假時鐘最多晚 4.03h，#171 量的）")
        LPLs = "LPL/2026 Season/Split 3"
        W21 = wk([(LPLs, "2026-09-14 07:58:00", "JDG", "AL", "3")])
        r21 = mk([("2026-09-14 12:01:00", "LPL", "JDG", "AL", 3)])
        eq(DC(r21, W21), ("ok", []), "㉙㉑ 補檔局假時鐘晚 4h03m ⇒ 預設 slack 吸掉、不誤報")
        st21, ms21 = DC(r21, W21, slack_h=4)
        eq((st21, any("LPL 2026-09-14 少 1 局（wiki 1、我們 0；" in m for m in ms21)), ("bad", True),
           "㉙㉑ 正控制：slack 4h ⇒ 同一份資料誤報（證明 4 不夠、擋下它的是 5；得到 %r）" % (ms21,))
        eq(DC(mk([]), W21)[0], "bad", "㉙㉑ 反例：那一局我們真的沒有 ⇒ 照報（slack 只放寬時間、不會把缺局吞掉）")
