...
with sync_playwright() as pw:
    ...
    r = pg.evaluate(BOARD)
    ok(r["n"] == 4 and r["gi"] == 3, "⭐ 開始＝新開一局（3 局 → 4 局，對決在第 4 局）", "n=%s gi=%s" % (r["n"], r["gi"]))
    ...
    ok(all(x == 0 for x in r["others"]), "⭐ 前面三局一格都沒被動到（點擊只落在對決局）", str(r["others"]))
    ...
    print("\n⑦ 雙方都系統＋自動出手（計時縮到 0.3 秒）")
    pg.evaluate("() => { window.__bpDuelDelayMs = 300; }")
    pg.click("#bpDuelMe0"); pg.wait_for_timeout(400)
    ok(pg.evaluate("() => V.bpDuel && V.bpDuel.me") == "0", "分段鈕「雙方都系統」→ 狀態記住")
    pg.click("#bpDuelStart"); pg.wait_for_timeout(300)
    r0 = pg.evaluate(BOARD)
    ok(r0["n"] == 5 and r0["gi"] == 4, "再按開始 → 又新開一局（第 5 局）", "n=%s gi=%s" % (r0["n"], r0["gi"]))
    t0 = time.time()
    ...
    print("\n⑧ 全局模式滿 5 局：開始不再加局，改清一局")
    pg.evaluate("() => { V.bpMode = 'global'; saveState(); render(); }"); pg.wait_for_timeout(500)
    pg.evaluate("() => { window.__bpDuelDelayMs = 60000; }")
    pg.click("#bpDuelStart"); pg.wait_for_timeout(600)
    r = pg.evaluate(BOARD)
    ok(r["n"] == 5 and r["filled"] == 0, "滿 5 局 → 局數不變、清了一局來打", "n=%s gi=%s" % (r["n"], r["gi"]))
